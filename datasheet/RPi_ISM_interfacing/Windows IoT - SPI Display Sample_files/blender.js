var WebBlender = function ($) {
    ...
    */
    var openBlend = function (postBody, parentElementId, environment, flight, clientType, hash, iframeOptions) {
        if (!htmlFetched) {
            ...
            // Check for flight in cookie as well as flight param
            var isEmberCliFlight = isFlightEnabled(flight, 'embercli:1')
                || isFlightEnabled(flight, 'embercliblenderjs:1')
                || (clientType === 'MusicVideoReading' && isFlightEnabled(flight, 'emberclimvr:1'));
    ...
    */
    var loadSingleItemPurchaseHtml = function (availabilityId, productId, skuId, options, auth, parentElementId, environment, flight, clientType, culture, market, cv, identityType, identityValue, mediaOptions, iframeOptions, purchaseServiceVersion, style) {
        var postBody = {
